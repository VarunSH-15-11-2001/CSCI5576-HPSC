  ...
  void PEsum( VD &field )
  {
    /*
        ...
        add from neighbours of that cell
          ...
          find the postion of the cell in the field vector
          
        identifying edge cells: if the i value is nRealx / nRealy then work bothwasys
    ...
    */
    ...
    // if(nei_e >= 0) { /* determine section of field to grab for edge (right colum)*/}
    // if(nei_w >= 0) { /* determine section of field to grab for edge (top row)*/}
    ...
    if ( nei_ne >= 0 )  err = MPI_Isend(&ne, 1 , MPI_DOUBLE , nei_ne , tag , MPI_COMM_WORLD , &request );
    if ( nei_se >= 0 )  err = MPI_Isend(&se, 1 , MPI_DOUBLE , nei_se , tag , MPI_COMM_WORLD , &request );
    if ( nei_nw >= 0 )  err = MPI_Isend(&nw, 1 , MPI_DOUBLE , nei_nw , tag , MPI_COMM_WORLD , &request );
    if ( nei_sw >= 0 )  err = MPI_Isend(&sw, 1 , MPI_DOUBLE , nei_sw , tag , MPI_COMM_WORLD , &request );
    ...
    if ( nei_ne >= 0 ) { err = MPI_Irecv(&rec_ne, 1 , MPI_DOUBLE , nei_ne , tag , MPI_COMM_WORLD , &request ); MPI_Wait(&request,&status); } 
    if ( nei_se >= 0 ) { err = MPI_Irecv(&rec_se, 1 , MPI_DOUBLE , nei_se , tag , MPI_COMM_WORLD , &request ); MPI_Wait(&request,&status); } 
    if ( nei_nw >= 0 ) { err = MPI_Irecv(&rec_nw, 1 , MPI_DOUBLE , nei_nw , tag , MPI_COMM_WORLD , &request ); MPI_Wait(&request,&status); } 
    if ( nei_sw >= 0 ) { err = MPI_Irecv(&rec_sw, 1 , MPI_DOUBLE , nei_sw , tag , MPI_COMM_WORLD , &request ); MPI_Wait(&request,&status); } 
